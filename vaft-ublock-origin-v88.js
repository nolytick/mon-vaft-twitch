...
(function() {
    ...
    'use strict';
    // Codex security-hardened snapshot based on ryanbr/TwitchAdSolutions
    // commit 232c5e95f915ccf59eac175044bb19ad24f84227 (vaft 68.5.1 / internal v87).
    // v88 adds a strict allow-list for main-thread fetches requested by the
    // injected worker. Normal vaft operation only needs Twitch's GQL endpoint.
    ...
    async function handleWorkerFetchRequest(fetchRequest) {
        // Security boundary: the evaluated Twitch worker shares this message
        // channel. Only relay the single endpoint and method used by vaft's
        // gqlRequest(); reject every other target before attaching captured
        // Twitch headers or invoking the page's original fetch implementation.
        let workerFetchUrl;
        try {
            workerFetchUrl = new URL(fetchRequest?.url);
        } catch {
            return {
                id: fetchRequest?.id,
                error: 'Blocked malformed worker fetch URL'
            };
        }
        const workerFetchMethod = String(fetchRequest?.options?.method || '').toUpperCase();
        if (workerFetchUrl.protocol !== 'https:' ||
            workerFetchUrl.hostname !== 'gql.twitch.tv' ||
            workerFetchUrl.port !== '' ||
            workerFetchUrl.pathname !== '/gql' ||
            workerFetchMethod !== 'POST') {
            console.warn('[AD DEBUG] Blocked unexpected worker fetch target');
            return {
                id: fetchRequest?.id,
                error: 'Blocked unexpected worker fetch target'
            };
        }
        ...
        try {
            const response = await window.realFetch(workerFetchUrl.href, {
